...
app.http("addLocation", {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            const { userID, location_name, location_address, place_id } = await request.json();
...
app.http("deleteLocation", {
    ...
    handler: async (request, context) => {
        ...
        try {
            ...
            const result = await client.db("SafeRoute").collection("saved_locations").deleteOne({ _id: location_id });
...
app.http("editLocation", {
    ...
    handler: async (request, context) => {
        ...
        try {
            await client.connect();
            const { location_id, location_name, location_address } = await request.json();
            const result = await client.db("SafeRoute").collection("saved_locations").updateOne({ _id: location_id }, { $set: { location_name, location_address } });
